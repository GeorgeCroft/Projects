...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/03/20 17:41:03 by gmura             #+#    #+#             */
/*   Updated: 2018/03/20 17:41:06 by gmura            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			parse_options(char *s, int *flags)
{
	...
	while (*(++s))
	{
		if ((n = ft_strchr_index("alRrtd1Ss", *s)) == -1)
			ls_error(s, USAGE);
		*flags |= (1 << n);
		if ((*s == 'l') || (*s == '1'))
...
int			ls_error(char *s, int error)
{
	if (error == USAGE)
	{
		...
		ft_putchar_fd(*s, 2);
		ft_putchar_fd('\n', 2);
		ft_putendl_fd("usage: ft_ls [-alRrtd1Ss] [file ...]", 2);
	}
